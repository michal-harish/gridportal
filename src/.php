...
function gridportal_main($output) {
    try {
        ...
        foreach($x->query("head/portlet[@id]") as $pid) {
            ...
            //map all params
            if (isset($portlets[$id]['config'])) $portlets[$id]['mapped'] = $portlets[$id]['config'];
            if (isset($portlets[$id]['required'])) foreach($portlets[$id]['required'] as $rk => $rc) {
        ...
        $portal_params = array();
        if (isset($portlet['mapped'])) foreach($_GET as $k=>$v) {
            $found = false;
                ...
                try {
                    ...
                    if ($new) {
                        ...
                        if (preg_match('/ajax/',$new->getAttribute("class"))) {
                            $attachObservers[$id][$fragment] = $maxage = isset($portlets[$id]['cache']['max-age']) ? $portlets[$id]['cache']['max-age'] : 1; 
                        }
        ...
        if ($attachObservers) {
            ...
            foreach($attachObservers as $id => $fragments) {
                $portletMaxAge = null;
                $portletAjaxFragments = '';
                foreach($fragments as $fragment => $fragmentMaxAge){
                    $portletMaxAge = $portletMaxAge === null ? $fragmentMaxAge : min($fragmentMaxAge, $portletMaxAge);
                    $portletAjaxFragments .= ($portletAjaxFragments ? "," : "" ) . $fragment;
                }
                $handler = "new observer(\"$id\",\"".$portletAjaxFragments."\",\"{$portlets[$id]['url']}\",\"{$portlets[$id]['cache']['last-modified']}\",\"$portletMaxAge\");";
                log::info($handler, "Portlet `$id`");
                $js .= "\n $handler\n";
            }
...
function prepare_portlet_request(array &$portlet) {
    ...
    if (isset($portlet['action']) && $_SERVER['REQUEST_METHOD'] != 'POST') { 
